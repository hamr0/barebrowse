...
| **Ad / tracker URL noise + load drag** | v0.10.0: CDP `Network.setBlockedURLs` with ~120 curated patterns covering Google/FB/Amazon/MS/Adobe ads+analytics, major SaaS analytics + session-replay stacks, content-rec, supply-side ad networks, marketing automation. Default on for launched browsers, off in attach mode. Shrinks ARIA snapshots and speeds loads. `opts.blockUrls` extends; `opts.blockAds: false` opts out | Launched |
